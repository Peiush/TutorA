...
const CourseSchema = z.object({
  ...
  durationHours: z.coerce.number().positive("Duration must be greater than 0.").optional(),
  lectureCount: z.coerce.number().int().positive("Lecture count must be greater than 0.").optional(),
...
async function saveCourse(_state: CourseFormState, formData: FormData, existingId?: string): Promise<CourseFormState> {
  ...
  const validated = CourseSchema.safeParse({
    ...
    lectureCount: formData.get("lectureCount") || undefined,
  ...
  const data = {
    ...
    instructorId: d.instructorId && d.instructorId !== "not-decided" ? d.instructorId : null,
    ...
    lectureCount: d.lectureCount ?? null,
