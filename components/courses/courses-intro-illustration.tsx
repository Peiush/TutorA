...
import { CheckIcon, LayersIcon, UserCheckIcon, VideoIcon, MicIcon, CameraIcon, PhoneOffIcon } from "@/components/courses/course-icons";
import { courseCategories } from "@/lib/mock-courses";

// Split out from courses-intro.tsx and lazy-loaded (ssr: false, no eager import)
// because this is purely decorative (aria-hidden, hidden below the lg breakpoint)
// but was otherwise pulling ScrollTrigger + the DrawSVG/MotionPath plugins into
// the same chunk as the hero text, delaying first paint on every device —
// including phones, where this never even renders.
...
export function CoursesIntroIllustration() {
    ...
    () => {
      ...
      mm.add("(prefers-reduced-motion: no-preference)", () => {
        gsap
          ...
          .from(".ci-stop-1", { autoAlpha: 0, x: 30, y: -16, duration: 0.5, ease: "power3.out" })
          ...
          .fromTo(".ci-connector-path-1", { drawSVG: "0%" }, { drawSVG: "100%", duration: 0.6, ease: "power2.inOut" }, "-=0.1")
          .to(
            ".ci-connector-dot-1",
  ...
  return (
    <div ref={rootRef} className="hidden lg:flex flex-col" aria-hidden>
