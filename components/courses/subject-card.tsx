...
function SubjectPill({ tone, children }: { tone: { light: string; text: string }; children: ReactNode }) {
  return (
    <span
        ...
        <div className="flex flex-col gap-1.5 p-4 flex-1">
          {(subject.curriculum || subject.gradeLevel) && (
            <div className="flex gap-1.5 flex-wrap mb-0.5">
              {subject.curriculum && <SubjectPill tone={colors}>{subject.curriculum}</SubjectPill>}
              {subject.gradeLevel && (
                <SubjectPill tone={{ light: "var(--color-accent-100)", text: "var(--color-accent-800)" }}>
                  {subject.gradeLevel}
                </SubjectPill>
              )}
            </div>
          ...
          {subject.subtitle && (
            <p
              className="text-[12.5px] leading-snug line-clamp-2 m-0"
              style={{ color: "color-mix(in srgb, var(--color-text) 70%, transparent)" }}
            >
              {subject.subtitle}
            </p>
          )}
          <div className="flex items-center gap-1 mt-auto pt-1 font-[var(--font-heading)]">
      ...
      >
        ...
        {subject.whatYoullLearn.length > 0 && (
          <ul className="flex flex-col gap-1.5 m-0 p-0 list-none">
            {subject.whatYoullLearn.slice(0, 3).map((item) => (
