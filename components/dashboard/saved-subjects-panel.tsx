...
import { SavedSubjectRow, SavedSubjectRowData } from "@/components/dashboard/saved-subject-row";

export function SavedSubjectsPanel({ subjects }: { subjects: SavedSubjectRowData[] }) {
  return (
    <div className="card elev-sm gap-4 p-[clamp(18px,3vw,26px)]">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-[19px]">Saved subjects</h2>
      ...
      {subjects.length === 0 ? (
        <div className="flex flex-col items-center text-center gap-3 py-10 px-4">
          ...
          <h3 className="text-[18px]">No saved subjects yet</h3>
          <p className="text-[14px] max-w-[38ch]" style={{ color: "color-mix(in srgb, var(--color-text) 66%, transparent)" }}>
            Bookmark subjects from a grade-level filter on Courses and they&rsquo;ll show up here.
          </p>
        ...
        <StaggerReveal className="flex flex-col gap-2.5" stagger={0.06} y={14}>
          {subjects.map((s) => (
            <SavedSubjectRow key={s.subjectId} subject={s} />
          ))}
        </StaggerReveal>
      )}
    </div>
