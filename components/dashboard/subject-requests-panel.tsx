...
import { SubjectRequestRow } from "@/components/dashboard/subject-request-row";
import { SubjectRequestRowData } from "@/components/dashboard/subject-request-status";

export function SubjectRequestsPanel({ requests }: { requests: SubjectRequestRowData[] }) {
  return (
    <div className="card elev-sm gap-4 p-[clamp(18px,3vw,26px)]">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-[19px]">Your subject requests</h2>
      ...
      {requests.length === 0 ? (
        <div className="flex flex-col items-center text-center gap-3 py-10 px-4">
          ...
          <h3 className="text-[18px]">No subject requests yet</h3>
          <p className="text-[14px] max-w-[38ch]" style={{ color: "color-mix(in srgb, var(--color-text) 66%, transparent)" }}>
            Send a request from a grade-level filter on Courses and our team will follow up with next steps.
          </p>
        ...
        <StaggerReveal className="flex flex-col gap-2.5" stagger={0.06} y={14}>
          {requests.map((r) => (
            <SubjectRequestRow key={r.id} request={r} />
          ))}
        </StaggerReveal>
      )}
    </div>
