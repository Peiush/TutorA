// Calling-code metadata for the phone country picker. Order roughly follows
// global usage frequency for the most common entries, then alphabetical.
export type Country = {
  iso2: string;
  name: string;
  dialCode: string;
...
/** Splits a stored phone value like "+91 9876543210" into country + national number. */
export function parsePhoneValue(value: string | null | undefined): { country: Country; national: string } {
  const fallback = findCountryByIso2("IN")!;
  if (!value) return { country: fallback, national: "" };

  const digits = value.trim().replace(/^\+/, "");
  if (!digits) return { country: fallback, national: "" };

  // Longest dial code match first (e.g. "1876" for Jamaica before "1" for the US/Canada).
  const sorted = [...COUNTRIES].sort((a, b) => b.dialCode.length - a.dialCode.length);
  const match = sorted.find((c) => digits.startsWith(c.dialCode));
  if (!match) return { country: fallback, national: digits.replace(/[^\d]/g, "") };

  const rest = digits.slice(match.dialCode.length).replace(/^[\s-]+/, "");
  return { country: match, national: rest.replace(/[^\d]/g, "") };
}

export function formatPhoneValue(country: Country, national: string): string {
