# Content Brief: A-Level Chemistry
...
**Hub page:** `/subjects/a-level-chemistry` (`https://www.tutora.it.com/subjects/a-level-chemistry`)
...
- No existing course cross-link in `lib/subject-course-links.ts` despite an A-Level Preparation course existing on the site (`a-level-preparation-course-6ede4ec7`) — recommend adding a light secondary cross-link at Step 3c, framed the same way as the GCSE/IGCSE pattern ("part of our full A-Level coverage").

### Winning Outline

**H1:** A-Level Chemistry tutor — 1:1 A-Level Chemistry Tutoring on TutorA
**URL Slug:** `/subjects/a-level-chemistry` (unchanged)
**Target Word Count:** ~950 words

1. **Intro / value prop** (~110 words) — Primary keyword "A-Level Chemistry tutor" in the first 100 words. State plainly: matched, vetted 1:1 tutors for this exact exam, live sessions, not a self-paced course.
2. **What A-Level Chemistry covers** (~110 words, H2) — Real, generically verifiable facts about the exam/curriculum (not TutorA-proprietary claims):
   - A-Levels are UK qualifications typically studied over two years (Years 12–13, ages 16–18), graded A*–E, and widely used for university admissions.
   - Step 3a confirmed zero exact-URL overlap vs. both GCSE and IGCSE Chemistry.
3. **Tutors who teach A-Level Chemistry on TutorA** (~180-250 words, H2 + repeatable bio-card structure) — **Populate via live query, not placeholder copy.** Query `TutorSubject` joined on `subjectId` for this subject directly (real relation, not prefix matching). Card fields per tutor: name, headshot, real credential/experience note, years tutoring this subject. Currently 0 tutors are linked — ship an honest "we don't have a dedicated A-Level Chemistry tutor listed yet, tell us what you need and we'll find one" state instead of an empty-looking grid or fabricated bios.
...
- **What grade do I need in A-Level Chemistry for university?** — Requirements vary by course and institution — check your target university's published entry requirements directly rather than relying on a general rule.
- **How is A-Level Chemistry different from GCSE Chemistry?** — It's a two-year, significantly more advanced course used for university admissions, with more mathematical and mechanistic depth than GCSE.
- **When should A-Level Chemistry tutoring start?** — Many students begin at the start of Year 12 to build a strong foundation before exam-focused Year 13 sessions.
- **Does A-Level Chemistry tutoring help with practical/coursework elements?** — Coverage depends on the tutor matched to you — mention any practical endorsement or coursework component when requesting a tutor.
9. **CTA** (~40 words) — "Request a A-Level Chemistry tutor" → `/request-a-tutor`.

### Recommended Meta Tags

**Title**
A-Level Chemistry tutor Online — 1:1 Match | TutorA

**Meta Description**
Get matched with a vetted A-Level Chemistry tutor on TutorA. Live 1:1 sessions, transparent pricing shown before booking. Request a tutor today.

### Unique Angle and Information Gain
Full standalone status backed by Step 3a's zero-overlap finding across the whole GCSE/IGCSE/A-Level Chemistry triad — the cleanest confirmation in the entire study.
...
- From `/subjects` listing → this page, anchor: "A-Level Chemistry tutoring"
- This page → `/subjects/gcse-chemistry`, anchor: "Preparing to move up from GCSE? See GCSE Chemistry"
- This page → `/request-a-tutor`, anchor: "Request an A-Level Chemistry tutor"
