...
**Hub page:** `/subjects/ib-math` (`https://www.tutora.it.com/subjects/ib-math`)
...
- No cross-link needed to IB Physics/Chemistry given zero SERP overlap — this page can be written with full confidence as its own standalone hub, only linking up to the IB Diploma umbrella page.

### Winning Outline

**H1:** IB Math tutor — 1:1 IB Mathematics Tutoring on TutorA
**URL Slug:** `/subjects/ib-math` (unchanged)
**Target Word Count:** ~950 words

1. **Intro / value prop** (~110 words) — Primary keyword "IB Math tutor" in the first 100 words. State plainly: matched, vetted 1:1 tutors for this exact exam, live sessions, not a self-paced course.
2. **What IB Math covers** (~110 words, H2) — Real, generically verifiable facts about the exam/curriculum (not TutorA-proprietary claims):
   - IB Diploma Math is offered in two strands — Analysis & Approaches (AA) and Applications & Interpretation (AI) — each available at Higher Level (HL) or Standard Level (SL).
   - Step 3a found a fully separate provider ecosystem from IB Physics/Chemistry — zero overlap, the strongest possible confirmation of standalone standing.
3. **Tutors who teach IB Math on TutorA** (~180-250 words, H2 + repeatable bio-card structure) — **Populate via live query, not placeholder copy.** Query `TutorSubject` joined on `subjectId` for this subject directly (real relation, not prefix matching). Card fields per tutor: name, headshot, real credential/experience note, years tutoring this subject. Currently 1 real tutor would populate this section.
...
- **What's the difference between IB Math AA and AI?** — AA (Analysis & Approaches) is more traditionally calculus/algebra-focused; AI (Applications & Interpretation) emphasizes real-world modeling and technology use. Mention which one your course covers when requesting a tutor.
- **HL or SL — does it matter for tutoring?** — Yes — mention your level (Higher or Standard) when requesting a tutor so sessions target the right depth.
- **How is IB Math scored?** — As part of the overall IB Diploma's 45-point scale, with internal assessment (exploration) and external exam components.
- **Do I need IB Math for university?** — Requirements vary by course and institution, and by whether AA or AI is required for specific degree programs — check your target university's published entry requirements.
9. **CTA** (~40 words) — "Request a IB Math tutor" → `/request-a-tutor`.

### Recommended Meta Tags

**Title**
IB Math tutor Online — 1:1 Match | TutorA

**Meta Description**
Get matched with a vetted IB Math (AA/AI, HL/SL) tutor on TutorA. Live 1:1 sessions, transparent pricing. Request a tutor today.

### Unique Angle and Information Gain
AA vs. AI, HL vs. SL specificity is real information gain — most generic "IB Math tutor" pages don't distinguish, and Step 3a's research shows dedicated providers do.
...
- From `/subjects` listing → this page, anchor: "IB Math tutoring"
- This page → `/subjects/international-baccalaureate-ib`, anchor: "Looking for other IB subjects?"
- This page → `/request-a-tutor`, anchor: "Request an IB Math tutor"
