...
**Hub page:** `/subjects/ap-biology` (`https://www.tutora.it.com/subjects/ap-biology`)
...
- No sibling subject in the catalog — standalone hub, no cannibalization risk.

### Winning Outline

**H1:** AP Biology tutor — 1:1 AP Biology Tutoring on TutorA
**URL Slug:** `/subjects/ap-biology` (unchanged)
**Target Word Count:** ~950 words

1. **Intro / value prop** (~110 words) — Primary keyword "AP Biology tutor" in the first 100 words. State plainly: matched, vetted 1:1 tutors for this exact exam, live sessions, not a self-paced course.
2. **What AP Biology covers** (~110 words, H2) — Real, generically verifiable facts about the exam/curriculum (not TutorA-proprietary claims):
   - AP Biology emphasizes data analysis and experimental design alongside content knowledge, more so than a typical high school biology course.
   - Scored 1–5; frequently used for intro-biology college credit.
3. **Tutors who teach AP Biology on TutorA** (~180-250 words, H2 + repeatable bio-card structure) — **Populate via live query, not placeholder copy.** Query `TutorSubject` joined on `subjectId` for this subject directly (real relation, not prefix matching). Card fields per tutor: name, headshot, real credential/experience note, years tutoring this subject. Currently 1 real tutor would populate this section.
...
- **Is AP Biology mostly memorization?** — Less than it used to be — the current exam weights data analysis and experimental-design questions heavily alongside content recall.
- **How is AP Biology scored?** — 1 to 5, combining multiple-choice and free-response sections.
- **Does AP Biology help with pre-med prep?** — It's a common early building block for students considering pre-med tracks, though it isn't a substitute for college-level biology coursework.
9. **CTA** (~40 words) — "Request a AP Biology tutor" → `/request-a-tutor`.

### Recommended Meta Tags

**Title**
AP Biology tutor Online — 1:1 Match | TutorA

**Meta Description**
Work 1:1 with a vetted AP Biology tutor on TutorA. Live sessions, transparent pricing, real subject-specific help. Request a tutor today.

### Unique Angle and Information Gain
Standalone hub with room for genuinely subject-specific FAQ and data-analysis-skill framing that the current shared 3-item list can't provide.
...
- From `/subjects` listing → this page, anchor: "AP Biology tutoring"
- This page → `/request-a-tutor`, anchor: "Request an AP Biology tutor"
