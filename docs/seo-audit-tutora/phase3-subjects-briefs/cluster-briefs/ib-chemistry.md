...
**Hub page:** `/subjects/ib-chemistry` (`https://www.tutora.it.com/subjects/ib-chemistry`)
...
- Cross-link to IB Physics as "IB Sciences" siblings, same reasoning as the IB Physics page.

### Winning Outline

**H1:** IB Chemistry tutor — 1:1 IB Chemistry Tutoring on TutorA
**URL Slug:** `/subjects/ib-chemistry` (unchanged)
**Target Word Count:** ~900 words

1. **Intro / value prop** (~110 words) — Primary keyword "IB Chemistry tutor" in the first 100 words. State plainly: matched, vetted 1:1 tutors for this exact exam, live sessions, not a self-paced course.
2. **What IB Chemistry covers** (~110 words, H2) — Real, generically verifiable facts about the exam/curriculum (not TutorA-proprietary claims):
   - IB Chemistry is offered at Higher Level (HL) or Standard Level (SL), with an Internal Assessment (IA) component.
   - Step 3a found the same 5-domain overlap pattern with IB Physics — shared providers, but zero exact-URL overlap.
3. **Tutors who teach IB Chemistry on TutorA** (~180-250 words, H2 + repeatable bio-card structure) — **Populate via live query, not placeholder copy.** Query `TutorSubject` joined on `subjectId` for this subject directly (real relation, not prefix matching). Card fields per tutor: name, headshot, real credential/experience note, years tutoring this subject. Currently 0 tutors are linked — ship an honest "we don't have a dedicated IB Chemistry tutor listed yet, tell us what you need and we'll find one" state instead of an empty-looking grid or fabricated bios.
...
- **HL or SL — does it matter?** — Yes — mention your level when requesting a tutor.
- **Does IB Chemistry tutoring help with the Internal Assessment (IA)?** — Coverage depends on the tutor matched to you — mention that you need IA support when requesting a tutor.
- **Am I also taking IB Physics?** — Many IB science students take both — see our IB Physics page too.
9. **CTA** (~40 words) — "Request a IB Chemistry tutor" → `/request-a-tutor`.

### Recommended Meta Tags

**Title**
IB Chemistry tutor Online — 1:1 Match | TutorA

**Meta Description**
Get matched with a vetted IB Chemistry tutor on TutorA. Live 1:1 sessions, HL/SL coverage. Request a tutor today.

### Unique Angle and Information Gain
Same honest "IB Sciences" framing as IB Physics — mirrors real student enrollment patterns without collapsing two distinct SERP-validated pages into one.
...
- From `/subjects` listing → this page, anchor: "IB Chemistry tutoring"
- This page → `/subjects/ib-physics`, anchor: "Also taking IB Physics?"
- This page → `/subjects/international-baccalaureate-ib`, anchor: "Looking for other IB subjects?"
- This page → `/request-a-tutor`, anchor: "Request an IB Chemistry tutor"
