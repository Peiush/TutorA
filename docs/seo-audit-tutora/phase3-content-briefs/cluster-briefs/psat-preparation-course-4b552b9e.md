...
**Hub page:** `/courses/psat-preparation-course-4b552b9e` (`https://www.tutora.it.com/courses/psat-preparation-course-4b552b9e`)
**File / slug:** `psat-preparation-course-4b552b9e`
...
| 5 | TestPrepInsight / BestColleges | Third-party "best PSAT tutor" rankings/comparisons | Listicle | TutorA has no comparison or ranking-style trust signal |
...
- **Quality gap:** Thin/generic copy is shared sitewide via one FAQ block; none of it is PSAT-specific.

### Winning Outline

**H1:** PSAT tutor — 1:1 PSAT Tutoring on TutorA
**URL Slug:** `/courses/psat-preparation-course-4b552b9e` (unchanged — this is a rewrite, not a new URL)
...
1. **Intro / value prop** (~120 words) — Primary keyword "PSAT tutor" in the first 100 words. State plainly what TutorA offers: matched, vetted 1:1 PSAT tutors, live sessions, not a self-paced course. No fabricated stats.
2. **What PSAT tutoring on TutorA includes** (~120 words, H2) — definition-box style. Use secondary keyword "PSAT tutoring online" in the H2. Cover format (live, 1:1, online), typical session structure, how matching works.
3. **Credentialed tutor bios** (~250 words, H2 + repeatable bio card structure) — **NEEDS REAL INPUT.** Do not fabricate. Structure only:
   ...
   - Years tutoring PSAT specifically
...
5. **Pricing** (~90 words, H2) — Answers "PSAT tutoring cost" honestly: pricing varies by tutor and is shown before booking (this matches the real site FAQ in `lib/course-faqs.ts` — do not invent a flat price).
...
- **PSAT tutoring cost** — commercial intent. Adjacent commercial-informational query; answer as FAQ entry on hub page rather than new page.
- **PSAT tutoring for high schoolers** — commercial intent. Audience-segment variant; fold into hub page's audience/FAQ section.
- **how much does PSAT tutoring cost** — informational-commercial intent. Pricing FAQ; fold into hub FAQ block.
- **PSAT study plan** — informational intent. Content-gap FAQ opportunity; low commercial intent but supports hub page depth.
- **when to start PSAT prep** — informational intent. Content-gap FAQ opportunity.
   - Plus one light-touch resource mention for: “PSAT practice test”, “PSAT test dates”, “what is the PSAT” — one or two sentences max, linking out to the official test body/free resource rather than trying to rank for it directly.
10. **CTA** (~40 words) — "Request a PSAT tutor" button linking to `/request-a-tutor`, secondary link to `/find-a-tutor`.

### Recommended Meta Tags

**Title** (52 chars)
PSAT tutor Online — Book a Vetted 1:1 Match | TutorA

**Meta Description** (130 chars)
Work 1:1 with a vetted PSAT tutor on TutorA. Personalised live sessions, transparent pricing, real results. Request a tutor today.
...
- Real, verifiable tutor credentials (not invented) — degree/certification, years of PSAT-specific experience
...
- From `/courses` (hub/listing page) → this page, anchor: "PSAT tutoring"
- This page → `/request-a-tutor`, anchor: "Request a PSAT tutor" (primary CTA)
- This page → `/find-a-tutor`, anchor: "Browse PSAT tutors"
