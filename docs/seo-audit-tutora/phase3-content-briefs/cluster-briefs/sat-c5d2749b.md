...
**Hub page:** `/courses/sat-c5d2749b` (`https://www.tutora.it.com/courses/sat-c5d2749b`)
**File / slug:** `sat-c5d2749b`
...
| 5 | TestPrepInsight / BestColleges | Third-party "best SAT tutor" rankings/comparisons | Listicle | TutorA has no comparison or ranking-style trust signal |
...
- **Quality gap:** Thin/generic copy is shared sitewide via one FAQ block; none of it is SAT-specific.

### Winning Outline

**H1:** SAT tutor — 1:1 SAT Tutoring on TutorA
**URL Slug:** `/courses/sat-c5d2749b` (unchanged — this is a rewrite, not a new URL)
...
1. **Intro / value prop** (~120 words) — Primary keyword "SAT tutor" in the first 100 words. State plainly what TutorA offers: matched, vetted 1:1 SAT tutors, live sessions, not a self-paced course. No fabricated stats.
2. **What SAT tutoring on TutorA includes** (~120 words, H2) — definition-box style. Use secondary keyword "SAT tutoring online" in the H2. Cover format (live, 1:1, online), typical session structure, how matching works.
3. **Credentialed tutor bios** (~250 words, H2 + repeatable bio card structure) — **NEEDS REAL INPUT.** Do not fabricate. Structure only:
   ...
   - Years tutoring SAT specifically
...
5. **Pricing** (~90 words, H2) — Answers "SAT tutoring cost" honestly: pricing varies by tutor and is shown before booking (this matches the real site FAQ in `lib/course-faqs.ts` — do not invent a flat price).
...
- **SAT tutoring cost** — commercial intent. Adjacent commercial-informational query; answer as FAQ entry on hub page rather than new page.
- **SAT tutoring for high schoolers** — commercial intent. Audience-segment variant; fold into hub page's audience/FAQ section.
- **how much does SAT tutoring cost** — informational-commercial intent. Pricing FAQ; fold into hub FAQ block.
- **SAT study plan** — informational intent. Content-gap FAQ opportunity; low commercial intent but supports hub page depth.
- **when to start SAT prep** — informational intent. Content-gap FAQ opportunity.
   - Plus one light-touch resource mention for: “SAT practice test”, “SAT test dates”, “what is the SAT” — one or two sentences max, linking out to the official test body/free resource rather than trying to rank for it directly.
10. **CTA** (~40 words) — "Request a SAT tutor" button linking to `/request-a-tutor`, secondary link to `/find-a-tutor`.

### Recommended Meta Tags

**Title** (51 chars)
SAT tutor Online — Book a Vetted 1:1 Match | TutorA

**Meta Description** (130 chars)
TutorA matches you with a vetted SAT tutor for live 1:1 sessions. Transparent pricing shown before booking. Request a tutor today.
...
- Real, verifiable tutor credentials (not invented) — degree/certification, years of SAT-specific experience
...
- From `/courses` (hub/listing page) → this page, anchor: "SAT tutoring"
- This page → `/request-a-tutor`, anchor: "Request a SAT tutor" (primary CTA)
- This page → `/find-a-tutor`, anchor: "Browse SAT tutors"
