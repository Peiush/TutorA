# Content Brief: A Level

**Mode:** IMPROVE (rewrite of existing live page) — not a new page
**Hub page:** `/courses/a-level-preparation-course-6ede4ec7` (`https://www.tutora.it.com/courses/a-level-preparation-course-6ede4ec7`)
**File / slug:** `a-level-preparation-course-6ede4ec7`
...
### Search Intent
Primarily **commercial** ("A Level tutor", "A Level tutoring online", "A Level prep course") — someone ready to hire a 1:1 tutor or enrol in a prep course, comparing providers. Secondary **commercial-local** ("A Level tutor near me") and **commercial-trust** ("A Level score guarantee") intent. Google rewards long-form Service/Hybrid pages here, not listing cards: Real SERP for 'A Level tutor' / 'A Level tutoring online' is dominated by UK/international tutoring services that foreground tutor credentials (MyTutor -- 1-in-8 acceptance rate, TutorChase -- 'IB examiners', Keystone Tutors, GoStudent, Sherpa -- DBS-checked, exam-board coverage AQA/Edexcel/OCR/Cambridge). No thin product-listing pages rank -- confirms the prior SXO finding for A Level.
...
| 5 | TestPrepInsight / BestColleges | Third-party "best A Level tutor" rankings/comparisons | Listicle | TutorA has no comparison or ranking-style trust signal |
...
- **Quality gap:** Thin/generic copy is shared sitewide via one FAQ block; none of it is A Level-specific.

### Winning Outline

**H1:** A Level tutor — 1:1 A Level Tutoring on TutorA
**URL Slug:** `/courses/a-level-preparation-course-6ede4ec7` (unchanged — this is a rewrite, not a new URL)
...
1. **Intro / value prop** (~120 words) — Primary keyword "A Level tutor" in the first 100 words. State plainly what TutorA offers: matched, vetted 1:1 A Level tutors, live sessions, not a self-paced course. No fabricated stats.
2. **What A Level tutoring on TutorA includes** (~120 words, H2) — definition-box style. Use secondary keyword "A Level tutoring online" in the H2. Cover format (live, 1:1, online), typical session structure, how matching works.
3. **Credentialed tutor bios** (~250 words, H2 + repeatable bio card structure) — **NEEDS REAL INPUT.** Do not fabricate. Structure only:
   ...
   - Years tutoring A Level specifically
...
5. **Pricing** (~90 words, H2) — Answers "A Level tutoring cost" honestly: pricing varies by tutor and is shown before booking (this matches the real site FAQ in `lib/course-faqs.ts` — do not invent a flat price).
...
- **A Level tutoring cost** — commercial intent. Pricing FAQ.
- **A Level tutoring for year 10 students** — commercial intent. Audience-segment FAQ variant.
- **how much does A Level tutoring cost** — informational-commercial intent. Pricing FAQ.
- **A Level revision timetable** — informational intent. Content-gap FAQ opportunity.
   - Plus one light-touch resource mention for: “A Level past papers”, “A Level syllabus”, “what is A Level” — one or two sentences max, linking out to the official test body/free resource rather than trying to rank for it directly.
10. **CTA** (~40 words) — "Request a A Level tutor" button linking to `/request-a-tutor`, secondary link to `/find-a-tutor`.

### Recommended Meta Tags

**Title** (55 chars)
A Level tutor Online — Book a Vetted 1:1 Match | TutorA

**Meta Description** (133 chars)
Work 1:1 with a vetted A Level tutor on TutorA. Personalised live sessions, transparent pricing, real results. Request a tutor today.
...
- Real, verifiable tutor credentials (not invented) — degree/certification, years of A Level-specific experience
...
- From `/courses` (hub/listing page) → this page, anchor: "A Level tutoring"
- This page → `/request-a-tutor`, anchor: "Request a A Level tutor" (primary CTA)
- This page → `/find-a-tutor`, anchor: "Browse A Level tutors"
