...
**Hub page:** `/courses/igcse-preparation-course-5ed7859f` (`https://www.tutora.it.com/courses/igcse-preparation-course-5ed7859f`)
**File / slug:** `igcse-preparation-course-5ed7859f`
...
### Search Intent
Primarily **commercial** ("IGCSE tutor", "IGCSE tutoring online", "IGCSE prep course") — someone ready to hire a 1:1 tutor or enrol in a prep course, comparing providers. Secondary **commercial-local** ("IGCSE tutor near me") and **commercial-trust** ("IGCSE score guarantee") intent. Google rewards long-form Service/Hybrid pages here, not listing cards: Real SERP for 'IGCSE tutor' / 'IGCSE tutoring online' is dominated by UK/international tutoring services that foreground tutor credentials (MyTutor -- 1-in-8 acceptance rate, TutorChase -- 'IB examiners', Keystone Tutors, GoStudent, Sherpa -- DBS-checked, exam-board coverage AQA/Edexcel/OCR/Cambridge). No thin product-listing pages rank -- confirms the prior SXO finding for IGCSE.
...
| 5 | TestPrepInsight / BestColleges | Third-party "best IGCSE tutor" rankings/comparisons | Listicle | TutorA has no comparison or ranking-style trust signal |
...
- **Quality gap:** Thin/generic copy is shared sitewide via one FAQ block; none of it is IGCSE-specific.

### Winning Outline

**H1:** IGCSE tutor — 1:1 IGCSE Tutoring on TutorA
**URL Slug:** `/courses/igcse-preparation-course-5ed7859f` (unchanged — this is a rewrite, not a new URL)
...
1. **Intro / value prop** (~120 words) — Primary keyword "IGCSE tutor" in the first 100 words. State plainly what TutorA offers: matched, vetted 1:1 IGCSE tutors, live sessions, not a self-paced course. No fabricated stats.
2. **What IGCSE tutoring on TutorA includes** (~120 words, H2) — definition-box style. Use secondary keyword "IGCSE tutoring online" in the H2. Cover format (live, 1:1, online), typical session structure, how matching works.
3. **Credentialed tutor bios** (~250 words, H2 + repeatable bio card structure) — **NEEDS REAL INPUT.** Do not fabricate. Structure only:
   ...
   - Years tutoring IGCSE specifically
...
5. **Pricing** (~90 words, H2) — Answers "IGCSE tutoring cost" honestly: pricing varies by tutor and is shown before booking (this matches the real site FAQ in `lib/course-faqs.ts` — do not invent a flat price).
...
- **IGCSE tutoring cost** — commercial intent. Pricing FAQ.
- **IGCSE tutoring for year 10 students** — commercial intent. Audience-segment FAQ variant.
- **how much does IGCSE tutoring cost** — informational-commercial intent. Pricing FAQ.
- **IGCSE revision timetable** — informational intent. Content-gap FAQ opportunity.
   - Plus one light-touch resource mention for: “IGCSE past papers”, “IGCSE syllabus”, “what is IGCSE” — one or two sentences max, linking out to the official test body/free resource rather than trying to rank for it directly.
10. **CTA** (~40 words) — "Request a IGCSE tutor" button linking to `/request-a-tutor`, secondary link to `/find-a-tutor`.

### Recommended Meta Tags

**Title** (53 chars)
IGCSE tutor Online — Book a Vetted 1:1 Match | TutorA

**Meta Description** (131 chars)
Work 1:1 with a vetted IGCSE tutor on TutorA. Personalised live sessions, transparent pricing, real results. Request a tutor today.
...
- Real, verifiable tutor credentials (not invented) — degree/certification, years of IGCSE-specific experience
...
- From `/courses` (hub/listing page) → this page, anchor: "IGCSE tutoring"
- This page → `/request-a-tutor`, anchor: "Request a IGCSE tutor" (primary CTA)
- This page → `/find-a-tutor`, anchor: "Browse IGCSE tutors"
